...
def get_release_info(force=False):
    ...
    with _release_lock:
        ...
        if force or stale:
            ...
            # Notificación de nueva versión eliminada
...
# Telegram bot eliminado

def generate_token():
    return secrets.token_urlsafe(32)

# verify_telegram_auth eliminado
...
def register():
    ...
    try:
        ...
        # chat_id = username if username.lstrip("-").isdigit() else username # Telegram eliminado
...
def register_verify():
    ...
    try:
        ...
        record = tokens_col.find_one({"username": username, "type": "register"})
...
def login():
    ...
    try:
        ...
        token   = generate_token()
        expires = utcnow() + timedelta(minutes=20)
        tokens_col.delete_many({"username": username, "type": "login"})
        tokens_col.insert_one({"username": username, "token": token, "expires": expires, "type": "login"})

        # chat_id = username if username.lstrip("-").isdigit() else username # Telegram eliminado
# Telegram login message removed
...
def login_verify():
    ...
    try:
        token_input = request.form.get("token", "").strip()
        record = tokens_col.find_one({"username": username, "type": "login"})
...
# /api/v1/telegram/id route removed
...
def api_generate_download_token():
    ...
    dl_tokens_col.insert_one({
        "token": token,
        "issued_to": request.api_user["username"],
        "expires": expires,
        "created_at": utcnow()
    })
...
# Telegram bot send functions removed
...
def oauth_userinfo():
    ...
    user = users_col.find_one({"username": token_data["user_id"]})
    if not user:
